...
func UpdateFollow(ctx context.Context, userID, followedUser uint, status *int) error {
	...
	return DB.WithContext(ctx).Model(&Follow{}).Where("user_id = ? and followed_user = ?", userID, followedUser).
		Updates(params).Error
}

// DeleteFollow delete follow info
func DeleteFollow(ctx context.Context, userID uint, followedUser uint) error {
	return DB.WithContext(ctx).Where("user_id = ? and followed_user = ? ", userID, followedUser).Delete(&Follow{}).Error
}
...
func QueryFollow(ctx context.Context, userID int64, status, limit, offset int) ([]*Follow, int64, error) {
	...
	if status == 1 {
		conn = DB.WithContext(ctx).Model(&Follow{}).Where("user_id = ?", userID)
	} else { // query for followers
		conn = DB.WithContext(ctx).Model(&Follow{}).Where("followed_user = ?", userID)
	}
